...
4. **Fetch the assembly rules.** Run, in order:
   ...
   - `wip-plumbing template show bundle/assemble`
...
6. **Validate.** Run `wip-plumbing intake validate --kind bundle <manifest>`. On
...
7. **Explode (inline).** Run the same fan-out `/wip:intake` drives for a
   ...
   - **Apply the lead.** Strip the bundle-only keys; the lead is the
     `lead-as` kind. For `amendment`, `wip-plumbing intake apply --kind amendment
   ...
   - **Apply each child**, in `depends-on` order: seed an amendment whose
     ...
     explicit directive hint, then shape + `wip-plumbing intake apply` it. A child
