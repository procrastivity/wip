...
| [0023](./0023-vendored-role-provenance-and-two-axis-drift-detection.md) | Vendored role provenance stamped in a `.claude/agents/wip/.provenance.json` sidecar (bytes stay pristine); drift split into two orthogonal axes vs the vendor-time `baseline_hash` — `upstream_advanced` ⟂ `locally_modified` + divergence direction (`upstream-behind` never auto-syncs); update path is additive flags `setup agents --status` / `--sync [--force] [--dry-run]` sharing one classifier; `doctor` fan-in gated `source: vendored`; amends ADR-0015 (closes Q-05.4) + ADR-0020 |
...
| [0025](./0025-role-centric-runtime-selection-and-duo-backend.md) | **role** replaces **tier** (`small/medium/large`) as the runtime-selection request axis (role is the only signal, no residual capability hint); the `medium→large` ladder becomes an opt-in per-role **escalation target** (`builder`→`builder-escalated`); `role→runtime` maps live in `.wip.yaml features.<backend>` (`solo.agent_tools` / `task.models` / `duo.presets`), Duo-only concepts (providers/presets/extra_args) stay out of the shared contract; a **Duo backend** delegates `role→preset` (identity-by-default), hard-errors at preflight when Duo is unreachable; amends ADR-0007 (axis) / 0012 (prep gate) / 0013 (Duo + tier→model deferrals graduate) |
