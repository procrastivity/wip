...
var (
	...
	syntheticV3 = migration{version: 9, name: "annotations", stmts: []string{
	...
	syntheticV4 = migration{version: 10, name: "node-review-note", stmts: []string{
	...
	brokenV3 = migration{version: 9, name: "half-applied", stmts: []string{
	...
	brokenV4 = migration{version: 10, name: "no-such-table", stmts: []string{
		`ALTER TABLE no_such_table ADD COLUMN nothing TEXT`,
	}}
)
...
func TestASyntheticMigrationBacksUpAppliesAndKeepsEverything(t *testing.T) {
	...
	at := time.Now()
	migrated, err := h.reopen(through(syntheticV3), 9)
	if err != nil {
		t.Fatalf("reopen under v2: %v", err)
	}

	// --- the migration was applied, and the store records it -----------------
	if got := migrated.SchemaVersion(); got != 9 {
		t.Errorf("the reopened store reports v%d, want v9", got)
	}
	wantMigrationLedger(migrated, "after v1 -> v9", through(syntheticV3))
	...
	direct := newHarnessAt(t, filepath.Join(t.TempDir(), "wip.db"), through(syntheticV3), 9)
...
func TestAStoreFromANewerBinaryIsRefusedAndNotDowngraded(t *testing.T) {
	...
	h := newHarnessAt(t, path, through(syntheticV3), 9)
	richHistory(h)
	before := h.snapshotProjection()
	log := h.rowsOf("events", "")
	...
	sound, err := h.reopen(through(syntheticV3), 9)
	if err != nil {
		t.Fatalf("reopen under v2 after two refused opens: %v", err)
	}
	if got := sound.SchemaVersion(); got != 9 {
		t.Errorf("the store is at v%d after two refused opens, want v9", got)
	}
...
func TestAMigrationThatFailsPartwayLeavesNothingBehind(t *testing.T) {
	...
	log := h.rowsOf("events", "")
	at := time.Now()
	_, err := h.reopen(through(brokenV3), 9)
	refusalMentions(t, "a synthetic migration whose second statement names no table", err, "migration v9")
	...
	if got := after.SchemaVersion(); got != latestVersion(shipped()) {
		t.Errorf("the store reports v%d after a failed v9, want v%d", got, latestVersion(shipped()))
	}
	...
	wantSameSchema(t, "after a v9 that failed partway", schemaShape(t, after.Store), schemaShape(t, directV2.Store))
	wantMigrationLedger(after, "after a v9 that failed", shipped())
...
func TestAllOrNothingIsPerMigrationAndNotPerOpen(t *testing.T) {
	...
	_, err := h.reopen(reg, 10)
	refusalMentions(t, "a v10 that names no table", err, "migration v10")

	// v5 landed and stayed; v6 did not.
	stopped, err := h.reopen(reg, 9)
	if err != nil {
		t.Fatalf("reopen at v5 after a failed v6: %v", err)
	}
	if got := stopped.SchemaVersion(); got != 9 {
		t.Errorf("the store is at v%d after v9 succeeded and v10 failed, want v9", got)
	}
	wantMigrationLedger(stopped, "after v9 succeeded and v10 failed", through(syntheticV3))
...
func TestTheBackupAFailedMigrationLeftRestoresTheStore(t *testing.T) {
	...
	_, err := h.reopen(through(brokenV3), 9)
	refusalMentions(t, "a synthetic migration that fails partway", err, "migration v9")
...
func TestABackupNeverOverwritesTheOneAlreadyThere(t *testing.T) {
	...
	// instant.
	fixed, err := h.reopen(through(syntheticV3), 9)
	if err != nil {
		t.Fatalf("a store whose migration was fixed did not open: %v", err)
	}
	if got := fixed.SchemaVersion(); got != 9 {
		t.Errorf("the retried migration reached v%d, want v9", got)
	}
...
func TestTwoMigrationsInOneOpenApplyInOrderUnderOneBackup(t *testing.T) {
	...
	at := time.Now()
	migrated, err := h.reopen(reg, 10)
	if err != nil {
		t.Fatalf("reopen under v3: %v", err)
	}
	if got := migrated.SchemaVersion(); got != 10 {
		t.Errorf("one open applied up to v%d, want v10", got)
	}
	wantMigrationLedger(migrated, "after v1 -> v10 in one open", through(syntheticV3, syntheticV4))
	...
	direct := newHarnessAt(t, filepath.Join(t.TempDir(), "wip.db"), reg, 10)
	...
	atTwo, err := stepwise.reopen(reg, 9)
	if err != nil {
		t.Fatalf("reopen under v2: %v", err)
	}
	atThree, err := atTwo.reopen(reg, 10)
	if err != nil {
		t.Fatalf("reopen under v3: %v", err)
	}
	...
	wantBackupName(t, stepwiseSidecars[1], 9, at)
}
