...
var (
	...
	syntheticV3 = migration{version: 8, name: "annotations", stmts: []string{
	...
	syntheticV4 = migration{version: 9, name: "node-review-note", stmts: []string{
	...
	brokenV3 = migration{version: 8, name: "half-applied", stmts: []string{
	...
	brokenV4 = migration{version: 9, name: "no-such-table", stmts: []string{
		`ALTER TABLE no_such_table ADD COLUMN nothing TEXT`,
	}}
)
...
func TestASyntheticMigrationBacksUpAppliesAndKeepsEverything(t *testing.T) {
	...
	at := time.Now()
	migrated, err := h.reopen(through(syntheticV3), 8)
	if err != nil {
	...
	if got := migrated.SchemaVersion(); got != 8 {
		t.Errorf("the reopened store reports v%d, want v8", got)
	}
	wantMigrationLedger(migrated, "after v1 -> v8", through(syntheticV3))
	...
	direct := newHarnessAt(t, filepath.Join(t.TempDir(), "wip.db"), through(syntheticV3), 8)
...
func TestAStoreFromANewerBinaryIsRefusedAndNotDowngraded(t *testing.T) {
	...
	h := newHarnessAt(t, path, through(syntheticV3), 8)
	richHistory(h)
	before := h.snapshotProjection()
	log := h.rowsOf("events", "")
	...
	sound, err := h.reopen(through(syntheticV3), 8)
	if err != nil {
		t.Fatalf("reopen under v2 after two refused opens: %v", err)
	}
	if got := sound.SchemaVersion(); got != 8 {
		t.Errorf("the store is at v%d after two refused opens, want v8", got)
	}
...
func TestAMigrationThatFailsPartwayLeavesNothingBehind(t *testing.T) {
	...
	log := h.rowsOf("events", "")
	at := time.Now()
	_, err := h.reopen(through(brokenV3), 8)
	refusalMentions(t, "a synthetic migration whose second statement names no table", err, "migration v8")
	...
	if got := after.SchemaVersion(); got != latestVersion(shipped()) {
		t.Errorf("the store reports v%d after a failed v8, want v%d", got, latestVersion(shipped()))
	}
	...
	wantSameSchema(t, "after a v8 that failed partway", schemaShape(t, after.Store), schemaShape(t, directV2.Store))
	wantMigrationLedger(after, "after a v8 that failed", shipped())
...
func TestAllOrNothingIsPerMigrationAndNotPerOpen(t *testing.T) {
	...
	_, err := h.reopen(reg, 9)
	refusalMentions(t, "a v9 that names no table", err, "migration v9")

	// v5 landed and stayed; v6 did not.
	stopped, err := h.reopen(reg, 8)
	if err != nil {
		t.Fatalf("reopen at v5 after a failed v6: %v", err)
	}
	if got := stopped.SchemaVersion(); got != 8 {
		t.Errorf("the store is at v%d after v8 succeeded and v9 failed, want v8", got)
	}
	wantMigrationLedger(stopped, "after v8 succeeded and v9 failed", through(syntheticV3))
...
func TestTheBackupAFailedMigrationLeftRestoresTheStore(t *testing.T) {
	...
	_, err := h.reopen(through(brokenV3), 8)
	refusalMentions(t, "a synthetic migration that fails partway", err, "migration v8")
...
func TestABackupNeverOverwritesTheOneAlreadyThere(t *testing.T) {
	...
	// instant.
	fixed, err := h.reopen(through(syntheticV3), 8)
	if err != nil {
		t.Fatalf("a store whose migration was fixed did not open: %v", err)
	}
	if got := fixed.SchemaVersion(); got != 8 {
		t.Errorf("the retried migration reached v%d, want v8", got)
	}
...
func TestTwoMigrationsInOneOpenApplyInOrderUnderOneBackup(t *testing.T) {
	...
	at := time.Now()
	migrated, err := h.reopen(reg, 9)
	if err != nil {
		t.Fatalf("reopen under v3: %v", err)
	}
	if got := migrated.SchemaVersion(); got != 9 {
		t.Errorf("one open applied up to v%d, want v9", got)
	}
	wantMigrationLedger(migrated, "after v1 -> v9 in one open", through(syntheticV3, syntheticV4))
	...
	direct := newHarnessAt(t, filepath.Join(t.TempDir(), "wip.db"), reg, 9)
	...
	atThree, err := atTwo.reopen(reg, 9)
	if err != nil {
		t.Fatalf("reopen under v3: %v", err)
	}
