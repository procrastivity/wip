...
func TestRole_SpawnCloseList_BoundToDispatch(t *testing.T) {
	...
	r := runIn(t, dir, dbEnv, "plumbing", "role", "spawn", "builder")
	...
	r = runIn(t, dir, dbEnv, "plumbing", "role", "spawn", "builder", "--json")
	if r.exitCode != 0 {
	...
	r = runIn(t, dir, dbEnv, "plumbing", "role", "spawn", "reviewer")
	...
	r = runIn(t, dir, dbEnv, "plumbing", "role", "close", "builder", "--json")
	if r.exitCode != 0 {
...
func TestRole_AsRoleClaimIsBackedByAnOpenSpawn(t *testing.T) {
	...
	r := runIn(t, dir, dbEnv, "plumbing", "matter", "create", "--title", "As role", "--as-role", "researcher")
	if r.exitCode == 0 || !strings.Contains(r.stderr, "no open spawn") {
		t.Fatalf("unbacked claim: exit=%d stderr=%q", r.exitCode, r.stderr)
	}

	if r := runIn(t, dir, dbEnv, "plumbing", "role", "spawn", "researcher"); r.exitCode != 0 {
		t.Fatalf("role spawn: exit=%d stderr=%q", r.exitCode, r.stderr)
	}

	// The same command under the same claim now speaks as the role.
	r = runIn(t, dir, dbEnv, "plumbing", "matter", "create", "--title", "As role", "--as-role", "researcher", "--json")
	if r.exitCode != 0 {
	...
	r = runIn(t, dir, env, "plumbing", "step", "create", matter.Locator, "--title", "Born under a role", "--json")
	if r.exitCode != 0 {
	...
	if r := runIn(t, dir, dbEnv, "plumbing", "role", "close", "researcher"); r.exitCode != 0 {
		t.Fatalf("role close: exit=%d stderr=%q", r.exitCode, r.stderr)
	}
	r = runIn(t, dir, env, "plumbing", "step", "create", matter.Locator, "--title", "After the close")
	if r.exitCode == 0 || !strings.Contains(r.stderr, "no open spawn") {
...
func TestRole_GateConfigDrivesActivation_D14(t *testing.T) {
	...
	r := runIn(t, dir, dbEnv, "plumbing", "role", "spawn", "verifier")
	...
	if r := runIn(t, dir, dbEnv, "plumbing", "gate", "declare", "verified", "--scale", "step"); r.exitCode != 0 {
	...
	m := mustJSON[nodePayload](t, runIn(t, dir, dbEnv, "plumbing", "matter", "create", "--title", "Verify me", "--json").stdout)
	st := mustJSON[nodePayload](t, runIn(t, dir, dbEnv, "plumbing", "step", "create", m.Locator, "--title", "One step", "--json").stdout)
	r = runIn(t, dir, dbEnv, "plumbing", "gate", "close", "verified", st.ID)
	if r.exitCode == 0 || !strings.Contains(r.stderr, "owning role") {
		t.Fatalf("human closing verified: exit=%d stderr=%q", r.exitCode, r.stderr)
	}
	r = runIn(t, dir, dbEnv, "plumbing", "gate", "close", "verified", st.ID, "--as-role", "verifier")
	if r.exitCode != 0 {
	...
	r = runIn(t, dir, dbEnv, "plumbing", "gate", "close", "reviewed-local", m.ID, "--as-role", "verifier")
	...
	if r := runIn(t, dir, dbEnv, "plumbing", "gate", "declare", "ci-green", "--scale", "matter"); r.exitCode != 0 {
		t.Fatalf("gate declare ci-green: exit=%d stderr=%q", r.exitCode, r.stderr)
	}
	r = runIn(t, dir, dbEnv, "plumbing", "role", "spawn", "warden")
...
func TestRole_DispatchCloseReapsOpenRoles_E2E(t *testing.T) {
	...
	r := runIn(t, dir, dbEnv, "plumbing", "role", "spawn", "coordinator", "--json")
	if r.exitCode != 0 {
		t.Fatalf("role spawn: exit=%d stderr=%q", r.exitCode, r.stderr)
	}
	spawned := mustJSON[roleP](t, r.stdout)

	if r := runIn(t, dir, dbEnv, "plumbing", "dispatch", "close"); r.exitCode != 0 {
...
func TestRole_ResearcherJITWorkplanFlow(t *testing.T) {
	...
	m := mustJSON[nodePayload](t, runIn(t, dir, dbEnv, "plumbing", "matter", "create", "--title", "Unplanned work", "--json").stdout)
	...
	for _, title := range []string{"Survey the ground", "Do the work", "Prove it"} {
		r := runIn(t, dir, asResearcher, "plumbing", "step", "create", m.Locator, "--title", title, "--json")
		if r.exitCode != 0 {
	...
	if r := runIn(t, dir, asResearcher, "plumbing", "role", "close", "researcher"); r.exitCode != 0 {
		t.Fatalf("role close: exit=%d stderr=%q", r.exitCode, r.stderr)
	}
	...
	r := runIn(t, dir, dbEnv, "plumbing", "session", "--json")
	if r.exitCode != 0 {
