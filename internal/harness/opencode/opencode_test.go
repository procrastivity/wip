...
func TestGenerate_ProducesSkillMD(t *testing.T) {
	files, err := opencode.Generate(testManifest())
	if err != nil {
	...
	if _, ok := files[".claude-plugin/plugin.json"]; ok {
		t.Error("Generate: opencode has no plugin.json — that mechanism is claude-code's own")
	}
}
...
func TestGenerate_ProjectsHandAuthoredTextVerbatim(t *testing.T) {
	...
	judgment, err := asset.Resolve("templates/skills/opencode/judgment.md")
	if err != nil {
...
func TestGenerate_EverythingElseTracesBackToTheManifest(t *testing.T) {
	...
	files1, err := opencode.Generate(m1)
	if err != nil {
		t.Fatalf("Generate(m1): %v", err)
	}
	files2, err := opencode.Generate(m2)
	if err != nil {
	...
	judgment, _ := asset.Resolve("templates/skills/opencode/judgment.md")
...
func TestSkillsDir_RespectsOverride(t *testing.T) {
	t.Setenv(opencode.SkillsDirEnv, "/tmp/example-skills-dir")
	dir, err := opencode.SkillsDir()
	if err != nil {
...
func TestAvailable_OverrideMissing(t *testing.T) {
	t.Setenv(opencode.SkillsDirEnv, filepath.Join(t.TempDir(), "missing"))
	if opencode.Available() {
