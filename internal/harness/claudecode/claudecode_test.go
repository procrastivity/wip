...
func TestGenerate_ProducesSkillMDAndPluginJSON(t *testing.T) {
	...
	if _, ok := files[".claude-plugin/plugin.json"]; !ok {
		t.Error("Generate: missing .claude-plugin/plugin.json")
	}
}
...
func TestGenerate_ProjectsHandAuthoredTextVerbatim(t *testing.T) {
	...
	judgment, err := asset.Resolve("templates/skills/claude-code/judgment.md")
	if err != nil {
...
func TestGenerate_EverythingElseTracesBackToTheManifest(t *testing.T) {
	// Two manifests differing only in tool version and verb set; the
	// judgment and guidance text must be byte-identical across both
	// renders (the only hand-written strings), while the rest of SKILL.md
	// differs — confirming everything but those two blocks is derived.
	m1 := testManifest()
	m2 := testManifest()
	m2.Tool.Version = "9.9.9"
	m2.Verbs = append(m2.Verbs, manifest.Verb{Name: "next", Kind: surface.Plumbing, Description: "what's next"})

	files1, err := claudecode.Generate(m1)
	if err != nil {
		t.Fatalf("Generate(m1): %v", err)
	}
	files2, err := claudecode.Generate(m2)
	if err != nil {
	...
	judgment, _ := asset.Resolve("templates/skills/claude-code/judgment.md")
...
func TestGenerate_PluginJSONShape(t *testing.T) {
	files, err := claudecode.Generate(testManifest())
	if err != nil {
		t.Fatalf("Generate: %v", err)
	}
	...
	if err := json.Unmarshal(files[".claude-plugin/plugin.json"], &plugin); err != nil {
		t.Fatalf("plugin.json does not parse: %v", err)
	}
	if plugin.Name == "" || plugin.Version != "1.2.3" {
		t.Errorf("plugin.json = %+v, want a non-empty name and version %q", plugin, "1.2.3")
	}
}

func TestSkillsDir_RespectsOverride(t *testing.T) {
	t.Setenv(claudecode.SkillsDirEnv, "/tmp/example-skills-dir")
	dir, err := claudecode.SkillsDir()
	if err != nil {
