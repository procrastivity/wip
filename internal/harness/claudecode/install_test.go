...
func withSkillsDir(t *testing.T) string {
	t.Helper()
	dir := t.TempDir()
	t.Setenv(claudecode.SkillsDirEnv, dir)
	return dir
}
...
func TestInstall_WritesStampedTree(t *testing.T) {
	...
	dir, err := claudecode.Install(testManifest())
	if err != nil {
	...
	if _, err := os.Stat(filepath.Join(dir, ".claude-plugin", "plugin.json")); err != nil {
		t.Errorf(".claude-plugin/plugin.json missing after install: %v", err)
	}
...
func TestUninstall_RemovesExactlyTheStampedTree(t *testing.T) {
	...
	removedDir, err := claudecode.Uninstall()
	if err != nil {
...
func TestUninstall_NothingInstalled(t *testing.T) {
	...
	if _, err := claudecode.Uninstall(); err == nil {
...
func TestDrift_ReportsAddedVerbAfterInstall_ThenReinstallClearsIt(t *testing.T) {
	...
	// Simulate a verb registered since install (write-surface growing the
	// verb set concurrently, per the Brief).
	m2 := testManifest()
	...
	// A re-install brings the stamp current; Drift against the fresh stamp
	// must report nothing.
