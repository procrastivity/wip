...
func TestGenerate_ProducesSkillMD(t *testing.T) {
	files, err := amp.Generate(testManifest())
	if err != nil {
	...
	if _, ok := files[".claude-plugin/plugin.json"]; ok {
		t.Error("Generate: amp has no plugin.json — that mechanism is claude-code's own")
	}
}
...
func TestGenerate_ProjectsHandAuthoredTextVerbatim(t *testing.T) {
	...
	judgment, err := asset.Resolve("templates/skills/amp/judgment.md")
	if err != nil {
...
func TestGenerate_EverythingElseTracesBackToTheManifest(t *testing.T) {
	...
	judgment, _ := asset.Resolve("templates/skills/amp/judgment.md")
...
func TestSkillsDir_RespectsOverride(t *testing.T) {
	t.Setenv(amp.SkillsDirEnv, "/tmp/example-skills-dir")
	dir, err := amp.SkillsDir()
	if err != nil {
