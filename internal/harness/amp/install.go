...
func Install(m manifest.Manifest) (string, error) {
	...
	for path, data := range files {
		...
		if err := os.MkdirAll(filepath.Dir(full), 0o755); err != nil {
			return "", fmt.Errorf("amp: creating %q: %w", filepath.Dir(full), err)
		}
		if err := os.WriteFile(full, data, 0o644); err != nil {
			return "", fmt.Errorf("amp: writing %q: %w", full, err)
		}
	}
...
func Uninstall() (string, error) {
	...
	if _, err := os.Stat(dir); os.IsNotExist(err) {
		...
		return "", fmt.Errorf("amp: checking install dir %q: %w", dir, err)
	}
	...
	if err := os.RemoveAll(dir); err != nil {
		return "", fmt.Errorf("amp: removing %q: %w", dir, err)
	}
	return dir, nil
}
