...
func TestGenerate_ProducesSkillMD(t *testing.T) {
	files, err := devin.Generate(testManifest())
	if err != nil {
	...
	if _, ok := files[".claude-plugin/plugin.json"]; ok {
		t.Error("Generate: devin has no plugin.json — that mechanism is claude-code's own")
	}
}
...
func TestGenerate_ProjectsHandAuthoredTextVerbatim(t *testing.T) {
	...
	judgment, err := asset.Resolve("templates/skills/devin/judgment.md")
	if err != nil {
...
func TestGenerate_EverythingElseTracesBackToTheManifest(t *testing.T) {
	...
	files1, err := devin.Generate(m1)
	if err != nil {
		t.Fatalf("Generate(m1): %v", err)
	}
	files2, err := devin.Generate(m2)
	if err != nil {
	...
	judgment, _ := asset.Resolve("templates/skills/devin/judgment.md")
...
func TestSkillsDir_RespectsOverride(t *testing.T) {
	t.Setenv(devin.SkillsDirEnv, "/tmp/example-skills-dir")
	dir, err := devin.SkillsDir()
	if err != nil {
...
func TestAvailable_OverrideMissing(t *testing.T) {
	t.Setenv(devin.SkillsDirEnv, filepath.Join(t.TempDir(), "missing"))
	if devin.Available() {
