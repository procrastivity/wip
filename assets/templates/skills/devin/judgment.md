...
(`wip plumbing next`), to check where things stand (`wip plumbing status`), and to record a
...
`wip plumbing status`, `wip plumbing next`, and `wip plumbing session`. A session's first look at
the work is `wip plumbing refresh` then `wip plumbing status`. If a generated file
...
the same: `wip plumbing start` → `wip plumbing next --set` → do the work → `wip plumbing finding
add` for each decision the diff cannot show → `wip plumbing finish` → `wip plumbing next`
to see what's next, then `wip plumbing next --set` it or `wip plumbing next --clear` if it
